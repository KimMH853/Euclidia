...
const equalLengthLine = (coordinates: Coordinate[], shapes: Shape[]) => {
  ...
  if (coordA && coordB && coordC) {
    const lineLength = getDistanceBetweenCoordinates(
      coordB.x,
      coordB.y,
      coordC.x,
      coordC.y
    );
    const Alines = shapes.filter(
      (line) =>
        line.type === "line" &&
    ...
    const coordinateOnLine = coordinates.filter((coord) =>
      lineEquation.some(
        (line) =>
          makeComparableValue(line.gradient * coord.x + line.yIntercept) ===
          makeComparableValue(coord.y)
      )
    );

    const correctCoordinates = coordinateOnLine.filter((coord) => {
      const distanceFromA = getDistanceBetweenCoordinates(
        coord.x,
        coord.y,
        coordA.x,
        coordA.y
      ...
      return (
        makeComparableValue(distanceFromA) === makeComparableValue(lineLength)
      );
    });
